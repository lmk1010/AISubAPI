<template>
  <div
    :class="[
      'plan-card group relative flex flex-col overflow-hidden rounded-2xl transition-all',
      'hover:-translate-y-1 hover:shadow-2xl',
    ]"
  >
    <!-- Glass background layer -->
    <div class="absolute inset-0 rounded-2xl bg-white/80 backdrop-blur-xl dark:bg-dark-800/90" />
    <!-- Left accent strip -->
    <div :class="['absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl', accentClass]" />
    ...
    <div class="relative flex flex-1 flex-col p-5">
      ...
      <div class="mb-4">
        ...
        <h3 class="mt-1.5 truncate text-lg font-bold text-gray-900 dark:text-white">{{ plan.name }}</h3>
        <p v-if="plan.description" class="mt-0.5 text-xs leading-relaxed text-gray-500 dark:text-gray-400 line-clamp-2">
          {{ plan.description }}
        </p>
      </div>
      ...
      <div class="mb-4">
        <div class="flex items-baseline gap-1.5">
          <span :class="['text-sm font-medium', textClass]">¥</span>
      ...
      <div v-if="plan.features.length > 0" class="mb-4 space-y-1.5">
        <div v-for="feature in plan.features" :key="feature" class="flex items-start gap-2">
          ...
          <span class="text-xs leading-relaxed text-gray-600 dark:text-gray-300">{{ feature }}</span>
        </div>
      </div>
      ...
      <button
        type="button"
        :class="['w-full rounded-xl py-2.5 text-sm font-bold tracking-wide transition-all active:scale-[0.97]', btnClass]"
