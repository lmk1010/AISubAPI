...
func (h *ChannelHandler) GetLiteLLMSuggestions(c *gin.Context) {
	...
	for name, p := range items {
		...
		out = append(out, litellmSuggestionItem{
			...
			CacheWrite5mPrice: p.CacheCreationInputTokenCost,
			CacheWrite1hPrice: p.CacheCreationInputTokenCostAbove1hr,
...
func (h *ChannelHandler) ImportLiteLLMModels(c *gin.Context) {
	...
	for _, raw := range req.Models {
		...
		if litellm.CacheCreationInputTokenCost > 0 {
			...
			entry.CacheWrite5mPrice = &v
		}
		if litellm.CacheCreationInputTokenCostAbove1hr > 0 {
			v := litellm.CacheCreationInputTokenCostAbove1hr
			entry.CacheWrite1hPrice = &v
		}
